...
ZL_EXP_VOID mySDL_PollEvent(ZL_EXP_VOID * VM_ARG,ZL_EXP_INT argcount)
{
	...
	if(retnum != 0)
	{
		...
		else if(gameEvent.type == SDL_FINGERDOWN) //手指点击屏幕事件
		{
			ZL_EXP_BOOL flag=ZL_EXP_FALSE;
			ZENGL_EXPORT_MEMBLOCK motionblock = {0};
			ZENGL_EXPORT_MOD_FUN_ARG mblk_val = {ZL_EXP_FAT_NONE,{0}};
			mblk_val = zenglApi_GetMemBlock(VM_ARG,&memblock,5);
			if(mblk_val.type == ZL_EXP_FAT_MEMBLOCK)
				motionblock = mblk_val.val.memblock;
			else
			{
				zenglApi_CreateMemBlock(VM_ARG,&motionblock,0);
			...
			arg.type = ZL_EXP_FAT_INT;
			arg.val.integer = gameEvent.tfinger.x;
			zenglApi_SetMemBlock(VM_ARG,&motionblock,2,&arg);
			arg.val.integer = gameEvent.tfinger.y;
			zenglApi_SetMemBlock(VM_ARG,&motionblock,3,&arg);
			if(flag)
			{
				arg.type = ZL_EXP_FAT_MEMBLOCK;
				arg.val.memblock = motionblock;
				zenglApi_SetMemBlock(VM_ARG,&memblock,5,&arg);
			}
		}
...
/*sdlAndroidLog*/
ZL_EXP_VOID mySDL_AndroidLog(ZL_EXP_VOID * VM_ARG,ZL_EXP_INT argcount)
{
	ZENGL_EXPORT_MOD_FUN_ARG arg = {ZL_EXP_FAT_NONE,{0}};
	if(argcount != 1)
		zenglApi_Exit(VM_ARG,"sdlAndroidLog函数必须有且只有一个参数，函数原型：sdlAndroidLog(message);\n");
	...
	if(arg.type == ZL_EXP_FAT_STR)
	{
		...
		zenglApi_Exit(VM_ARG,"sdlAndroidLog函数的参数必须是字符串，表示日志内容");
	zenglApi_SetRetVal(VM_ARG,ZL_EXP_FAT_INT,ZL_EXP_NULL,0,0);
}
...
ZL_EXP_VOID mySDL_SetBkImg(ZL_EXP_VOID * VM_ARG,ZL_EXP_INT argcount)
{
	...
	if(arg.type == ZL_EXP_FAT_STR)
	{
		...
		//utf8_path = arg.val.str;
#endif
		//file = SDL_RWFromFile(utf8_path, "rb");
		//if((tmpBmp = IMG_Load_RW(file, 1)) == ZL_EXP_NULL)
...
extern DECLSPEC SDL_Window * SDLCALL SDL_MyGetWindowPtr();

/*sdlGetWindowSurface获取主窗口的表面指针*/
ZL_EXP_VOID mySDL_GetWindowSurface(ZL_EXP_VOID * VM_ARG,ZL_EXP_INT argcount)
{
	SDL_Window * tmpwindow = SDL_MyGetWindowPtr();
	SDL_Surface * oldSurface = gl_sdlLib_pScreen;
	int i;
	gl_sdlLib_pScreen = SDL_GetWindowSurface(tmpwindow);
	for(i=0;i<gl_sdlLib_PointsList.count;i++) //循环释放掉所有SDL指针
	{
		if(gl_sdlLib_PointsList.points[i].isvalid == ZL_EXP_TRUE && gl_sdlLib_PointsList.points[i].point == oldSurface)
		{
			gl_sdlLib_PointsList.points[i].point = gl_sdlLib_pScreen;
		}
	}
	zenglApi_SetRetVal(VM_ARG,ZL_EXP_FAT_INT,ZL_EXP_NULL,(ZL_EXP_LONG)gl_sdlLib_pScreen,0);
}
...
ZL_EXP_VOID mySDL_GetKeyState(ZL_EXP_VOID * VM_ARG,ZL_EXP_INT argcount)
{
	...
	memblock = arg.val.memblock;
	//keystate = (Uint8 *)SDL_GetKeyState(&numkey);
	/*arg.type = ZL_EXP_FAT_INT;
	for(i=1;i <= numkey;i++)
	{
